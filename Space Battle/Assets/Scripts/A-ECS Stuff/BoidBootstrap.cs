    ...
    public class BoidBootstrap : MonoBehaviour
{
    ...
    Entity CreateSmallBoid(Vector3 pos, Quaternion q, int boidId, float size)
    {
        ...
        entityManager.SetComponentData(boidEntity,
            new Boid() {boidId = boidId, mass = 1, maxSpeed = 100, maxForce = 400, weight = 200});
        ...
        entityManager.SetComponentData(boidEntity,
            new ObstacleAvoidance() {forwardFeelerDepth = 50, forceType = ObstacleAvoidance.ForceType.normal});
    ...
    Entity Create4PartBoid(Vector3 pos, Quaternion q, int boidId, float size, int parts)
    {
        ...
        entityManager.SetComponentData(boidEntity,
            new Boid()
            {
                boidId = boidId, mass = 1, maxSpeed = 100 * UnityEngine.Random.Range(0.9f, 1.1f), maxForce = 400,
                weight = 200
            });
        entityManager.SetComponentData(boidEntity, new Seperation());
        entityManager.SetComponentData(boidEntity, new Alignment());
        ...
        for (int i = 0; i < parts; i++)
        {
            ...
            entityManager.SetComponentData(spineEntity,
                new Spine() {parent = parentId, spineId = parentId + 1, offset = new Vector3(0, 0, -size)});
