...
namespace UnityEngine.UIExtras
{
    class SearchView : VisualElement
    {
        ...
        public SelectionType SelectionType { get => m_List.selectionType; set { m_List.selectionType = value; } }
    ...
    void OnItemsChosen(IEnumerable<object> selection)
    {
...
#else
        void OnListSelectionChanged(List<object> selection)
        {
            if (SelectionType == SelectionType.Single)
            {
                OnitemChosen(selection[0]);
            }
            else
            {
                // TBD.
        ...
        void OnitemChosen(object selection)
        {
            TreeNode<Item> node = selection as TreeNode<Item>;
            if (node.ChildCount == 0)
            {
                OnSelection?.Invoke(node.Value);
            }
