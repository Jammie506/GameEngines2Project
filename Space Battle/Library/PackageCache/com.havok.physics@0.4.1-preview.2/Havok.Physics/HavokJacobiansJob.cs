                    ...
                    for (int i = 0; i < curGrid->m_size; i++)
                    {
                        ...
                        while (range != null)
                        {
                            ...
                            while (reader.HasItems)
                            {
                                ...
                                // Populate all modifiableHeader and modifiableContact fields with values read from plugin
                                /// Important: all values that will be used in <paramref name="jobData"/> or written back after executing the job
                                /// need to be copied from the plugin's jacobians here!
#if ENABLE_UNITY_COLLECTIONS_CHECKS
                                // Set both buffers to 1s, try exposing potential usage of fields that were not populated with values from plugin
                                UnsafeUtility.MemSet(headerBuffer, 0xFF, JacobianHeader.CalculateSize(JacobianType.Contact, (JacobianFlags)0xff, 4));
                                UnsafeUtility.MemSet(contactBuffer, 0xFF, sizeof(ContactJacobian));
#endif
                                ...
                                {
                                    // Surface velocity needs just 3 components: 2 for linear (2d on object surface - dir0 and dir1) and 1 for angular (along the normal)
                                    float3 hpSurfaceVelocity = *hpHeader->accessSurfaceVelocity();
                                    Math.CalculatePerpendicularNormalized(modifiableContact.m_ContactJacobian->BaseJacobian.Normal, out float3 dir0, out float3 dir1);

                                    // Set the surface velocity on the header, to avoid setting the dirty (ModifiersChanged) flag
                                    modifiableHeader.m_Header->SurfaceVelocity = new SurfaceVelocity()
                                    {
                                        LinearVelocity = hpSurfaceVelocity.x * dir0 + hpSurfaceVelocity.y * dir1,
                                        AngularVelocity = hpSurfaceVelocity.z * modifiableContact.m_ContactJacobian->BaseJacobian.Normal
                                    };
                                }
